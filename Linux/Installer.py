...
def check_priv():
    ...
    if os.geteuid() != 0:
        error_handler("You do not have sufficient privileges to run this program. Please run 'sudo ./Installer.py' instead.")
...
class ImgWriter(Thread):
    ...
    def run(self):
        ...
        dd = Popen(['dd', 'status=noxfer', 'bs=1M', 'of=' + self.device], stdin=unzip.stdout, stderr=STDOUT)
